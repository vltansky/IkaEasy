if(typeof langs == "undefined") {
    var langs = {};
}

//English language
langs.es = {
    'ikaeasy_link': 'https://ikariam-easy.com/?ref=game',
    'movements':{
        all: 'todo',
        transport: 'transporte',
        plunder: 'saqueo',
        blockade: 'bloqueo',
        deployarmy: 'apostar tropas',
        deployfleet: 'apostar flotas',
        defend: 'defender',
        defend_port: 'defender puerto',
        occupy: 'ocupar',
        trade: 'cambiar'
    },
    'char_day': 'd',
    'char_hour': 'h',
    'char_minute': 'm',
    'char_second': 's',
    'WillBeFullIn': 'lleno en',
    'per_day': "por dia",
    'per_week': "por semana",
    'per_month': "por mes",
    'left': "faltante",
    'hours': "horas",
    'days': "días",
    'months': "meses",
    'years': "años",
    'MessagePlaceholder': "CTRL + Enter para enviar mensaje",
    'ConfirmDowngrade': "Confirmar demolicion de edificio",
    'Circular_message': "Enviar circular",
    'Ally_members': "Alianza",
    'Attack_All': "Todos",
    'Attack_Half': "Mitad",
    'Attack_Nothing': "Ninguno",
    'Required': "Requerido:",
    'Next_Level': "Proximo Nivel:",
    'Messages': "Mensajes",
    'Agora': "Agora",
    'Treaty': "Tratados",
    'Alliance': "Muro",
    'Transport': "Transporte de bienes",
    'Send_General_Log': "Envar registro a aliados",
    'Send_General_Log_Header': "Ataques a miembros de la alianza",

    'options': {
        'transport': {
            'header': 'Opciones de transporte',
            'original': 'GameForge',
            'ikaeasy': 'IkaEasy'
        },
        'movement_tabs': {
            'header': 'Registro de movimientos',
            'enable': 'Activado',
            'disable': 'Desactivado'
        },
        'gold_per_hour': {
            'header': 'Consumo de oro',
            'enable': 'Activado',
            'disable': 'Desactivado'
        },
        'island_ap': {
            'header': 'Puntos de accion en la isla',
            'enable': 'Activado',
            'disable': 'Desactivado'
        },
        'development': {
            'overview': "IkaEasy es un proyecto no-comercial, <a href='https://github.com/swat-web/IkaEasy' target='_blank'>y de OpenSource</a>. Quieres ayudar? Contribuye en <a href='https://github.com/swat-web/IkaEasy' target='_blank'>GitHub</a> y reporta los bugs y errores en <a href='https://ikariam-easy.com/?ref=game' target='_blank'>Facebook</a>.<br>Desarrollar Ikariam Easy lleva mucho tiempo: fines de semana, tardes y, por supuesto, noches. Puedes agradecérmelo y ayudarme a mantenerme despierto comprandome un cafe :)"
        }
    },

    'ikalogs': {
        'save_log': 'Guardar registro',
        'analyze': 'Analizar',
        'each': 'Cada',
        'help_bw': 'Separar cada ronda con una coma, o un conjunto con un guión',

        'get_info': 'Obteniendo data',
        'get_round': 'Obteniendo registro',
        'saving': 'Guardando registro',

        'saving_success': 'El registro fue guardado con éxito.',
        'open_report': 'Abrir registro',

        'saving_failed': 'Se ha fallado al guardar el registro',
        'repeat': 'Reintentar',

        'auth': {
            'not_logged': 'No te has logeado en Ikalogs'
        },

        'types': {
            'full': 'Registro completo',
            'short': 'Version corta',
            'last': 'Ultima ronda',
            'each': 'Cada N rondas',
            'between': 'Rondas específicas'
        }
    },

    'Transporter': 'Transportar'
};