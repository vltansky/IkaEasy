...
function overrideResourcesReports() {
    ...
    for (i = 0; i < resourcesTables.length; ++i) {
        ...
        var tr = $("<tr/>");
        tr.append($("<td/>", {text: "Warehouse level "}));
        ...
        $(table).append(tr);
        $(table.children[0]).append($("<th/>", {class: "count", text: "Available to steal "}));
        for (var j = 1; j < table.children.length - 2; ++j) {
            var row = $(table.children[j]);
            row.append($("<td/>", {class: "count", text: "0"}));
        }
