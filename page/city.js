...
zJS.Page.city = {
    ...
    _getResearch : function() {
        zJS.Utils.getAjax('http://' + top.location.host + '/index.php?view=noViewChange&researchType=economy&templateView=researchAdvisor&actionRequest=' + zJS.Var.getActionRequest() + '&ajax=1', function(data){this._checkResearch(data[1][1]);}.bind(this));
    },
    ...
    _watcher : function() {
        ...
        var redAll = 0;
        if (this._watcher_values.pulley)   { redAll = 2; }
        if (this._watcher_values.geometry) { redAll = redAll + 4; }
        if (this._watcher_values.spirit)   { redAll = redAll + 8; }
    ...
    _checkResearch : function(data) {
        ...
        var redAll = 0;
        if (result['pulley'])   { redAll = 2; }
        if (result['geometry']) { redAll = redAll + 4; }
        if (result['spirit'])   { redAll = redAll + 8; }
        redAll = (100 - redAll) / 100;
